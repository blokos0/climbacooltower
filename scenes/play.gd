...
func setuproom() -> void:
	...
	$gamelayer/gameworldcontainer/gameworld/shader/camera.limit_top = global.leveldata["rooms"][room].rect.position.y * 32
	$gamelayer/gameworldcontainer/gameworld/shader/camera.limit_right = global.leveldata["rooms"][room].rect.position.x * 32 + global.leveldata["rooms"][room].rect.size.x * 32
